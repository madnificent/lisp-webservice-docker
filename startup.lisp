...
;;; do we have a base command to run?
(format t "~& >> docker loading eval command ... ~%")
(when (env-value :eval)
  (eval (read-from-string (env-value :eval) :eof-value t)))
(format t "~& >> finished running eval command ... ~%")

;;; getting swank up
(format t "~& >> docker starting swank ... ~%")
(ql:quickload :swank) ;; we need this toplevel
(setf swank::*LOOPBACK-INTERFACE* "0.0.0.0")
(swank:create-server :port 4005 :dont-close t)
(format t "~& >> docker finished starting swank ~%")
